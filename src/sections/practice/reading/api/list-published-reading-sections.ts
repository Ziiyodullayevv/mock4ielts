...
export type ReadingPracticeListItem = {
  difficulty: SectionDifficulty;
...
export async function listPublishedReadingSections(): Promise<ReadingPracticeSectionsResult> {
  ...
  return {
    items: sections.map((section, index) => ({
      ...
      href: `${paths.practice.reading.root}#section-${section.id}`,
      id: index + 1,
