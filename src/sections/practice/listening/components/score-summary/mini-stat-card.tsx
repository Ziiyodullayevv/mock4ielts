...
export function MiniStatCard({ accent, detail, label, percentage }: MiniStatCardProps) {
  return (
    <div className={`rounded-2xl p-5 ${PRACTICE_CARD_RING_GRAY_CLASS}`}>
      <div className="flex h-full items-center justify-center gap-3">
        <div className="relative shrink-0">
          ...
          <div className="pointer-events-none absolute inset-0 grid place-items-center">
            <span className="text-base font-semibold tracking-[-0.04em] text-stone-900 dark:text-white">
              {percentage}%
            </span>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <p className="text-base text-stone-700 dark:text-white/66">{label}</p>
          <p className="text-sm leading-6 text-stone-500 dark:text-white/44">{detail}</p>
        </div>
      </div>
    </div>
  );
